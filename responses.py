...
def _cur(amount, currency="SAR") -> str:
    try:
...
def fmt_low_stock_list(products: list, arabic: bool = False) -> str:
    ...
    for p in products:
        ...
        lines.append(f"{icon} {name} — {label} ({balance})")
...
def fmt_all_stock(products: list, arabic: bool = False) -> str:
    ...
    for p in products:
        ...
        lines.append(f"{icon} {name}: *{balance}*")
...
def fmt_sales_summary(invoices: list, period: str, arabic: bool = False) -> str:
    total = sum(float(inv.get("total") or 0) for inv in invoices)
    paid = sum(float(inv.get("paid") or 0) for inv in invoices)
    count = len(invoices)
    ...
    if arabic:
        return (
            ...
            f"  ✅ المحصّل: *{_cur(paid)}*\n"
            f"  ⏳ المتبقي: *{_cur(total - paid)}*"
        )
    return (
        f"💵 *Sales — {period}*\n\n"
        f"  🧾 Invoices: *{count}*\n"
        f"  💰 Total: *{_cur(total)}*\n"
        f"  ✅ Collected: *{_cur(paid)}*\n"
        f"  ⏳ Pending: *{_cur(total - paid)}*"
    )
...
def fmt_outstanding_invoices(invoices: list, arabic: bool = False) -> str:
    ...
    total_due = sum(float(inv.get("balance") or inv.get("amount_due") or 0) for inv in invoices)
    header = (
        f"📋 *الفواتير غير المدفوعة* ({len(invoices)})\n"
        f"الإجمالي المستحق: *{_cur(total_due)}*\n\n"
    ) if arabic else (
        f"📋 *Outstanding Invoices* ({len(invoices)})\n"
        f"Total due: *{_cur(total_due)}*\n\n"
    )
    lines = [header]
    for inv in invoices[:15]:
        client = inv.get("client_name") or inv.get("Client", {}).get("name") if isinstance(inv.get("Client"), dict) else inv.get("client_name") or "—"
        due = float(inv.get("balance") or inv.get("amount_due") or 0)
        inv_no = inv.get("number") or inv.get("id") or "—"
        date = (inv.get("date") or "")[:10]
        lines.append(f"• {client} — *{_cur(due)}* | #{inv_no} | {date}")
...
def fmt_suppliers(suppliers: list, arabic: bool = False) -> str:
    ...
    with_balance = [s for s in suppliers if float(s.get("balance") or 0) != 0]
    total_payable = sum(float(s.get("balance") or 0) for s in with_balance if float(s.get("balance") or 0) > 0)
...
def fmt_daily_summary(
    ...
    sales_total = sum(float(inv.get("total") or 0) for inv in today_invoices)
    sales_count = len(today_invoices)
    oos_count = len(out_of_stock)
    outstanding_total = sum(float(inv.get("balance") or inv.get("amount_due") or 0) for inv in outstanding)

    if arabic:
        msg = (
            f"☀️ *ملخص الصباح — {today}*\n\n"
            f"💵 *مبيعات اليوم*\n"
            f"  🧾 {sales_count} فاتورة | إجمالي: *{_cur(sales_total)}*\n\n"
            f"📦 *المخزون*\n"
            f"  {'🚫 ' + str(oos_count) + ' منتج نفد' if oos_count else '✅ كل المنتجات متوفرة'}\n\n"
            f"📋 *الفواتير المستحقة*\n"
            f"  {'⚠️ ' + _cur(outstanding_total) + ' مستحق' if outstanding else '✅ لا يوجد مستحقات'}"
        )
    else:
        msg = (
            f"☀️ *Morning Summary — {today}*\n\n"
            f"💵 *Today's Sales*\n"
            f"  🧾 {sales_count} invoices | Total: *{_cur(sales_total)}*\n\n"
            f"📦 *Stock*\n"
            f"  {'🚫 ' + str(oos_count) + ' products out of stock' if oos_count else '✅ All products in stock'}\n\n"
            f"📋 *Outstanding Invoices*\n"
            f"  {'⚠️ ' + _cur(outstanding_total) + ' due' if outstanding else '✅ No outstanding invoices'}"
        )
